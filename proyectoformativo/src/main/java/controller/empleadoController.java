...
public class empleadoController extends HttpServlet {
    ...
    private void login(HttpServletRequest req, HttpServletResponse resp) {
        if(req.getParameter("correo_electronico")!=null){
            r.setCorreoEmpleado(req.getParameter("correo_electronico"));
        }
        if(req.getParameter("password")!=null){
            r.setContrasenaUsuario(req.getParameter("password"));
        }
        try {
            List loginList=rd.login(r.getCorreoEmpleado(),r.getContrasenaUsuario());
            ...
            if (!loginList.isEmpty()) {
                ...
                //req.setAttribute("id_usuario", loginList.get(0));
                sesion.setAttribute("usuarioSesion", loginList.get(0));
                req.getRequestDispatcher("menu.jsp").forward(req, resp);
            }else{
                req.getRequestDispatcher("login.jsp").forward(req, resp);
            }

                System.out.println("Resultado es"+loginList);
                
            
            System.out.println("Se hizo la consulta el usuario (login)");
            //login(req, resp);
        } catch (Exception e) {
            System.out.println("Algo salio mal"+e.getMessage().toString());
        }
    }
